...
{
    Entry **buckets; // Mảng của các con trỏ buckets
    int capacity;    // Tổng số lượng các buckets
} HashTable;

// Khởi tạo HashTable với capacity bucket
HashTable *ht_create(int capacity);

// Giải phóng bộ nhớ (entries + buckets + struct)
...
// In key ra stdout (trên một dòng)
